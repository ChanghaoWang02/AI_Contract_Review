...
- 单条解析失败 → 标记 `risk=unknown`，继续下一条
- 单批次失败 → 该批次所有条目标记 `risk=unknown`
...
| 交互 | 行为 |
|------|------|
| 点击合同面板 [📄 对比] | 弹出 `<input type="file">` |
| 选择文件 + 视角 → 确定 | 开始 SSE 连接，弹窗打开 |
| 进度阶段 | 旋转 loading + 阶段文字 |
| 收到 `clause` 事件 | 左右两侧对应条款高亮 + 变更列表追加一行 |
| 点击变更列表条目 | 左右两侧滚动到对应条款 |
| 切换视角 | v1 不支持对比中切换。关闭弹窗后重新发起对比，选择不同视角即可 |
| 关闭弹窗 | 断开 SSE，清空状态 |
| 左右分栏 | 同步滚动 |
